defmodule Payeezy.TestHelpers do
  ...
  @spec simulate_service_response(any(), Plug.Conn.status(), String.t(), (Plug.Conn.t() -> boolean())) ::
          no_return()
  ...
  def simulate_balance_inquiry_and_purchase_success(bypass, balance) do
    Bypass.expect(bypass, fn conn ->
      ...
      case Poison.decode!(body)["transaction_type"] do
        "balance_inquiry" ->
          Plug.Conn.resp(conn, :ok, successful_balance_inquiry_response_string(balance))

        "purchase" ->
          Plug.Conn.resp(
            conn,
            :ok,
            successful_purchase_response_string(
              Poison.decode!(body)["amount"],
              "2.00",
              balance
            )
          )
      ...
      case Poison.decode!(body)["transaction_type"] do
        ...
        "purchase" ->
          Plug.Conn.resp(
            conn,
            :ok,
            purchase_failure_insufficient_funds(Poison.decode!(body)["amount"])
          )
      end
    end)
  end
  ...
  def successful_balance_inquiry_response_string(current_balance) do
    ~s(
      ...
      "credit_card_type":"Gift","previous_balance":"#{current_balance}","current_balance":"#{
      current_balance
    }"}}
    )
  end
  ...
  def successful_purchase_response_string(purchase_amount, current_balance, previous_balance) do
    ~s(
      ...
      "credit_card_type":"Gift","previous_balance":"#{previous_balance}","current_balance":"#{
      current_balance
    }"},
